...
@Component({
  selector: 'app-sign-home',
  templateUrl: './sign-home.page.html',
  styleUrls: ['./sign-home.page.scss'],
})
export class SignHomePage implements OnInit {
  ...
  fbLogin(){
    ...
    if(this.platform.is('cordova')){
      this.fb.login(['email']).then((response: FacebookLoginResponse) => {
        ...
        console.log(error);
        alert('facebook login error:' + JSON.stringify(error));
      });
    }else{
